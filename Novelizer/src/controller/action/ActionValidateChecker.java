...
public class ActionValidateChecker {
	...
	private boolean actionPreCheck(DBObject actionJSONData) {
		if(!actionHas(actionJSONData.get("id"))) return false;
		if(!actionHas(actionJSONData.get("type"))) return false;
		return true;
	}

	private boolean textActionIsValid(DBObject actionJSONData) {
		if(!actionHas(actionJSONData.get("text"))) return false;
		if(!actionHas(actionJSONData.get("characterId"))) return false;
		return true;
	}
	
	private boolean characterActionIsValid(DBObject actionJSONData) {
		if(!actionHas(actionJSONData.get("characterId"))) return false;
		if(!actionHas(actionJSONData.get("presetId"))) return false;
		if(!actionHas(actionJSONData.get("option"))) return false;
		if(!actionHas(actionJSONData.get("animation"))) return false;
		if(!actionHas(actionJSONData.get("position"))) return false;
		return true;
	}
